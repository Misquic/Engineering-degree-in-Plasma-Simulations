...
public:
    Source(Species& species, World& world, type_calc v_drift, type_calc den, std::string inlet_face) noexcept;
...
class ColdBeamSource: public Source{ //Z- FACE
protected:
    std::function<void()> sample_strategy;
...
class WarmBeamSource: public Source{ //Z- FACE
protected:
    std::function<void()> sample_strategy;
    type_calc T = 0;
...
public:
    /*constructor*/
    WarmBeamSource(Species& species, World& world, type_calc v_drift, type_calc den, type_calc T, std::string inlet_Face = "-z") noexcept;
