...
protected:
	...
	type_calc W_sigma_v_rel_max = 1e-14;	//some initial value for max term weight*sigma*v_rel
    ...
    void collide(type_calc3& vel_atom, type_calc3& vel_ele, bool& ionize, type_calc3& vel_new, type_calc sigma_coll);
...
public:
    MC_MEX_Ionization(Species& neutrals, Species& ions, Species& electrons, World& world, std::string coll_data_path = "data/Oxygen_momentum_transfer.txt", int freq_should_use_map = 10);

    virtual void apply(type_calc dt) noexcept override;
    void apply_map(type_calc dt) noexcept;
    void apply_vector_indexes(type_calc dt) noexcept;
    // void apply_vector_pointers(type_calc dt);
};
